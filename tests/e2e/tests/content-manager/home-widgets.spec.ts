...
test.describe('Homepage - Content Manager Widgets', () => {
  ...
  test('a user should see entries per locale in the last published entries widget', async ({
    ...
    await titleFieldEnglish.fill('West Ham post match analysis');
    await page.getByRole('button', { name: /publish/i }).click();
    await findAndClose(page, 'Published document');
    ...
    await titleFieldFrench.fill('Analyse post-match de West Ham');
    await page.getByRole('button', { name: /publish/i }).click();
    await findAndClose(page, 'Published document');
    ...
    const frenchEntry = recentlyPublishedWidget
      .getByRole('row')
      .filter({ hasText: 'Analyse post-match de West Ham' });
