...
test.describe(
  ...
  () => {
    ...
    test('Adding a component, DZ, and relation to an existing CT renders them in the CM editor', async ({
      ...
      // Three schema changes → three server restarts; the describe-level timeout isn't honored by
      // this Playwright version, so raise it per-test to avoid the default 90s cap under load.
