...
const ADMIN_DISPLAY_NAME = 'test testing';
const EDITOR_DISPLAY_NAME = 'editor testing';
const escapeRegExp = (value: string): string => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

const logoutAndLoginAs = async ({
  page,
  ...
  await page
    .getByRole('button', { name: new RegExp(escapeRegExp(currentUserDisplayName), 'i') })
    .click();
...
test.describe('Admin Tokens — Ownership', () => {
  ...
  test('Owner shown, Copy and Regenerate absent for non-owner', async ({ page }) => {
    ...
    await logoutAndLoginAs({
      page,
      currentUserDisplayName: ADMIN_DISPLAY_NAME,
      username: EDITOR_EMAIL_ADDRESS,
    ...
    await logoutAndLoginAs({
      page,
      currentUserDisplayName: EDITOR_DISPLAY_NAME,
...
test.describe('Admin Tokens — Permission Ceiling', () => {
  ...
  test('Super admin has no ceiling on Settings permissions', async ({ page }) => {
    const waitForPermissionsLayout = page.waitForResponse(
      (response) =>
        response.request().method() === 'GET' &&
        response.url().includes('/admin/permissions') &&
        response.ok()
    );
    ...
    await waitForPermissionsLayout;
