...
describe('Authenticated User', () => {
  ...
  describe('PUT /users/me', () => {
    ...
    describe('email uniqueness', () => {
      ...
      const createTrackedUser = async (userInfo) => {
        const user = await utils.createUser(userInfo);
      ...
      afterEach(async () => {
        await Promise.all(createdUserIds.splice(0).map((id) => utils.deleteUserById(id)));
      });

      test('allows changing to an unused email', async () => {
        const suffix = uniqueSuffix();
        const userA = await createTrackedUser({
          firstname: 'Uniq',
          lastname: 'UserA',
      ...
      test('returns validation error for malformed email before duplicate-email logic', async () => {
        const suffix = uniqueSuffix();
        const userEmail = `bad-email-${suffix}@strapi.io`;
        ...
        await rqBad.login({ email: userEmail });
      ...
      test('returns 400 when email is already used by another admin', async () => {
        const suffix = uniqueSuffix();
        const takenEmail = `user-b-me-${suffix}@strapi.io`;
      ...
      test('persists Yup transforms when updating profile data', async () => {
        const suffix = uniqueSuffix();
        ...
        const rqA = createRequest({ strapi });
        await rqA.login({ email: initialEmail });
