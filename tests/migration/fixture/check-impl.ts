...
type CheckErrors = { errors: string[] };
type CheckLines = { lines: string[]; errors: string[] };

function hasActiveEntry(activeEntries: ActiveEntry[], uid: string): boolean {
  return activeEntries.some((entry) => entry.uid === uid);
}

async function validateDocumentIdBackfill(strapi: Strapi): Promise<CheckErrors> {
  const errors = [];
...
async function validateDraftPublishPairingForUid(
  strapi: Strapi,
  uid: string,
  { label, localized = false }: { label: string; localized?: boolean }
): Promise<CheckErrors> {
  const errors = [];
...
async function validateDraftPublishPairing(
  strapi: Strapi,
...
async function validateRelationsPresence(
  strapi: Strapi,
  activeEntries: ActiveEntry[]
): Promise<CheckErrors> {
  const errors = [];
...
function validateRelationValue(value: unknown, path: string): string | null {
...
function validateComponentValue(
  component: Record<string, unknown> | null | undefined,
  componentUid: string | null | undefined,
  path: string,
  strapi: Strapi
): string[] {
  const errors = [];
...
async function validateEntityGraph(strapi: Strapi): Promise<CheckErrors> {
  const errors = [];
...
function collectMediaIds(value: unknown, path = ''): Array<string | number> {
  const ids = [];
  if (!value) return ids;
  if (Array.isArray(value)) {
    value.forEach((item, idx) => {
...
async function validateMediaParityForDp(strapi: Strapi, uid: string): Promise<CheckErrors> {
  const errors = [];
...
async function validateNestedComponentRelationParityForUid(
  strapi: Strapi,
  uid: string,
  label: string
): Promise<CheckErrors> {
...
async function verifyMigrationFixAtDbLevel(strapi: Strapi): Promise<CheckLines> {
  const out = [];
  const errors = [];
  try {
    const conn = strapi.db.connection;
    ...
    out.push(`  Verification skipped (${message}).`);
  }
  return { lines: out, errors };
}

async function validateJoinTableSourceParityForDp(
  strapi: Strapi,
  uid: string
): Promise<CheckErrors> {
  const errors = [];
...
async function validateRelationParityForDp(strapi: Strapi, uid: string): Promise<CheckErrors> {
  const errors = [];
  const contentType = strapi.contentTypes[uid];
  if (!contentType) return { errors };
