...
type DbClient = 'postgres' | 'mysql' | 'mariadb' | 'sqlite' | string;

function composeEnv(ctx: MigrationContext, extra: EnvRecord = {}): NodeJS.ProcessEnv {
 ...
 */
function nestedYarnInstallEnv(extra: EnvRecord = {}): NodeJS.ProcessEnv {
  return {
    ...process.env,
...
async function resolveDockerHostPorts(client: DbClient): Promise<void> {
...
function buildDatabaseEnvForClient(
...
function writeAppDotenv(ctx: MigrationContext, dbEnv: Record<string, string>): void {
...
`;
  ...
  fs.writeFileSync(path.join(ctx.V4_APP_DIR, '.env'), body);
}
...
type WorkspaceBuildOpts = {
  fullBuild?: boolean;
  skipBuild?: boolean;
};

async function runWorkspaceBuilds(
  repoRoot: string,
  { fullBuild, skipBuild }: WorkspaceBuildOpts
): Promise<void> {
  if (fullBuild) {
  ...
  if (!skipBuild) {
    const strapiPkg = path.join(repoRoot, 'packages', 'core', 'strapi', 'package.json');
    const databasePkg = path.join(repoRoot, 'packages', 'core', 'database', 'package.json');
