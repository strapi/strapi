...
// Load fixture via monorepo tsx so .ts modules resolve from plain node / tsx runners.
...
const { resolveContext } = monorepoRequire(
  path.join(repoRoot, 'tests/migration/fixture/resolve-context')
);
const { runChecks } = monorepoRequire(path.join(repoRoot, 'tests/migration/fixture/run-checks'));
