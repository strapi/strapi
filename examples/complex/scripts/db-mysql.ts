...
switch (command) {
  ...
  case 'snapshot':
    if (!snapshotName) {
      ...
      console.error('Usage: node --import tsx db-mysql.ts snapshot <name>');
      process.exit(1);
    }
  ...
  case 'restore':
    if (!snapshotName) {
      console.error('Error: Snapshot name is required');
      console.error('Usage: node --import tsx db-mysql.ts restore <name>');
      process.exit(1);
    }
    assertSafeSnapshotName(snapshotName);
  ...
  default:
    ...
    console.error(
      'Usage: node --import tsx db-mysql.ts <start|stop|snapshot|restore|wipe|check> [name]'
    );
    process.exit(1);
}
