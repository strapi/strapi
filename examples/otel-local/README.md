...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Jaeger**         | **Traces** — **Startup** root traces `strapi.startup` (via `start()`) or `strapi.startup.load` (via `load()` only), with nested register/bootstrap/listen and provider/DB sub-spans (`strapi.startup.provider.register.*`, `strapi.startup.bootstrap.db.*`, …); HTTP spans; nested Knex `db.*`; Content API **sanitize / validate**; **admin permissions** sanitize/validate; **document service** (`strapi.documents.*` spans / `strapi.document_service.operation.duration_ms` for every repository call — REST, admin Content Manager, plugins). Open [http://localhost:16686](http://localhost:16686) (no auth). Search by **service** name (e.g. `strapi-complex`). **`strapi develop` CLI** steps (admin webpack/vite, TS compile before fork) are **not** traced here — only the Node server process. |
