...
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Jaeger**         | **Traces** — **Startup** root span `strapi.startup` wraps register, bootstrap, listen, and (when using `strapi develop`) **CLI phases** between `load()` and `start()`: `strapi.startup.cli.watch_admin`, `strapi.startup.cli.generate_types`, `strapi.startup.cli.compile_typescript`, `strapi.startup.cli.init_file_watcher`. Sub-spans include `strapi.startup.register.*`, `strapi.startup.bootstrap.*`, `strapi.startup.listen`, provider registers, HTTP, Knex `db.*`, Content API, admin permissions, document service. Open [http://localhost:16686](http://localhost:16686). Search by **service** name (e.g. `strapi-complex`). |
