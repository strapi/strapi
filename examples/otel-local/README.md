...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Jaeger**         | **Traces** — **App (worker):** `strapi.startup`, `strapi.startup.cli.*` (post-`load()` develop work), **HTTP** `METHOD path`, **`strapi.http.route.*`**, **Knex** `db.*`, **`bcryptjs.*`**, Content API, document service. **`strapi develop` cluster primary:** when the same **`STRAPI_OTEL_*` env** is set, a **separate** service `<name>-cli-primary` sends **`strapi.cli.develop.primary`** (initial TS + admin production build) and **`strapi.cli.develop.primary.reload_typescript`** on watch reloads — this is **before** `strapi.load()` and is intentionally **not** nested under `strapi.startup` (different process / no cross-process trace link). Open [http://localhost:16686](http://localhost:16686). |
...
**`strapi develop` cluster primary** (compile + production admin build before the worker starts) uses the **same `STRAPI_OTEL_*` environment variables** as in `.env` (no `server.js` required). Disable with `STRAPI_OTEL_ENABLED=false` if you do not want those CLI traces.
...
- **`strapi.startup`** — **Root** trace for server boot. With `strapi develop`, `load()` runs register + bootstrap under this root; the root stays open until `start()` ends it. **`strapi.startup.cli.*`** spans cover work the **develop worker** does between `load()` and `listen()` (admin watch build, TypeScript generators, `tsc`, chokidar). A long gap before `strapi.startup.listen` was often untraced CLI work; check **`strapi.startup.cli.generate_types`** and **`strapi.startup.cli.compile_typescript`** first.
