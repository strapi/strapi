 ...
 */
const selectMenuEditorDomain = () => state => state.get(`${pluginId}_menuEditor`);
...
const SelectMenuItemsData = () =>
  createSelector(selectMenuEditorDomain(), substate => substate.get('menuItems'));

const SelectModifiedMenuItemsData = () =>
  createSelector(selectMenuEditorDomain(), substate => substate.get('modifiedMenuItems'));
