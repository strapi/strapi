...
module.exports = {
  ...
  /*
   * It converts GraphQL object params to a dotted query strings
   * @example
   *
   * params = { where: { posts: { isPublished: true } } }
   *
   * @result
   *
   * params = { where: { 'posts.isPublished': true } }
   *
   * NB: this only works with one level deep.
   */
  convertToQuery: (params) => {
    return _.reduce(params, (acc, value, parentKey) => {
      if (!_.isObject(value)) {
        acc[parentKey] = value;
        return acc;
      }
      ...
      return {
        ...acc,
        ..._.reduce(value, (_acc, _value, _key) => ({
          ..._acc,
          [`${parentKey}.${_key}`]: _value
        }), {})
      };
    }, {});
  },
    ...
    const resolver = (() => {
      ...
      return async (ctx, next) => {
        ...
        queryOpts.query = Object.assign(
          ...
          this.convertToQuery(ctx.params.where)
        );
