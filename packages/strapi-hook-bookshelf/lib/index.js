      ...
      initialize: async cb => {
        ...
        _.forEach(connections, (connection, connectionName) => {
          ...
          try {
            ...
            require(path.resolve(strapi.config.appPath, 'config', 'functions', 'bookshelf.js'))(ORM, connection);
          } catch (err) {
                    ...
                    new Promise(async resolve => {
                      ...
                      if (loadedModel.hasTimestamps) {
                        definition.attributes[
                          _.isString(loadedModel.hasTimestamps[0]) ? loadedModel.hasTimestamps[0] : 'created_at'
                        ] = {
                          type: 'timestamp',
                        };
                        definition.attributes[
                          _.isString(loadedModel.hasTimestamps[1]) ? loadedModel.hasTimestamps[1] : 'updated_at'
                        ] = {
                      ...
                      if (manyRelations && manyRelations.dominant) {
                        ...
                        const table =
                          ...
                          _.map(
                            _.sortBy([collection.attributes[manyRelations.via], manyRelations], 'collection'),
                            table => {
                              return _.snakeCase(
                ...
                switch (verbose) {
                  case 'hasOne': {
                    const FK = details.plugin
                      ? _.findKey(strapi.plugins[details.plugin].models[details.model].attributes, details => {
                          if (
                  ...
                  case 'hasMany': {
                    const columnName = details.plugin
                      ? _.get(
                          strapi.plugins,
                          `${details.plugin}.models.${globalId.toLowerCase()}.attributes.${details.via}.columnName`,
                          details.via,
                        )
                      : _.get(
                  ...
                  case 'belongsToMany': {
                    ...
                    const collectionName =
                      ...
                      _.map(_.sortBy([collection.attributes[details.via], details], 'collection'), table => {
                        return _.snakeCase(
                          // eslint-disable-next-line prefer-template
                          pluralize.plural(table.collection) + ' ' + pluralize.plural(table.via),
                        );
                    ...
                    if (
                      `${details.attribute}_${details.column}` === `${relationship.attribute}_${relationship.column}`
                    ) {
                  ...
                  case 'morphOne': {
                    ...
                    loadedModel[name] = function() {
                      return this.morphOne(GLOBALS[globalId], details.via, `${definition.collectionName}`).query(qb => {
                  ...
                  case 'morphMany': {
                    ...
                    loadedModel[name] = function() {
                      return this.morphMany(GLOBALS[globalId], details.via, `${definition.collectionName}`).query(
                        qb => {
                  ...
                  case 'belongsToManyMorph': {
                    const association = definition.associations.find(association => association.alias === name);
                    ...
                    const options = {
                      ...
                      [definition.collectionName]: function() {
                        return this.belongsTo(GLOBALS[definition.globalId], `${definition.collectionName}_id`);
                      },
                      related: function() {
                        return this.morphTo(
                          name,
