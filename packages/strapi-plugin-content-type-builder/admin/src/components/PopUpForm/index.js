...
class PopUpForm extends React.Component {
  ...
  createComponent = el => {
    if (get(el, ['inputDescription', 'params', 'link', 'children', 'type'], '') === 'FormattedMessage') {
      return (
        <FormattedMessage
          id={get(el, ['inputDescription', 'params', 'link', 'children', 'attr', 'id'], 'default')}
          defaultMessage=" "
        >
          {message =>
            React.createElement(
  ...
  renderInput = (item, key) => {
    ...
    let customBootstrapClass =
      item.type === 'textarea' ? 'col-md-8 offset-md-4 mr-md-5' : 'col-md-6 offset-md-6 mr-md-5';
    ...
    let value =
      !isEmpty(this.props.values) && includes(item.name, '.')
        ? get(this.props.values, [split(item.name, '.')[0], split(item.name, '.')[1]])
    ...
    return (
      <ModalFooter className={styles.modalFooter}>
        ...
        {popUpFormType !== 'contentType' && (
          <Button type="submit" onClick={onSubmit} className={styles.primaryAddShape}>
            <FormattedMessage id="content-type-builder.button.attributes.add" />
          </Button>
        )}
        <Button type="button" onClick={this.handleSubmit} className={styles.primary} id="continue">
          <FormattedMessage id={`content-type-builder.${buttonSubmitMessage}`} />
        </Button>{' '}
      </ModalFooter>
    );
  };
      ...
      <div className={styles.popUpForm}>
        <Modal isOpen={this.props.isOpen} toggle={this.props.toggle} className={`${styles.modalPosition}`}>
          ...
          <div className={styles.headerContainer}>
            <div className={styles.titleContainer}>{this.renderPopUpHeader()}</div>
            {navContainer}
          </div>
...
PopUpForm.propTypes = {
  ...
  overrideHandleBlurCondition: PropTypes.oneOfType([PropTypes.bool, PropTypes.func]),
  overrideRenderInput: PropTypes.oneOfType([PropTypes.bool, PropTypes.func]),
  overrideRenderInputCondition: PropTypes.oneOfType([PropTypes.bool, PropTypes.func]),
  popUpFormType: PropTypes.string,
  popUpHeaderNavLinks: PropTypes.array,
  popUpTitle: PropTypes.string.isRequired,
  renderCustomPopUpHeader: PropTypes.oneOfType([PropTypes.func, PropTypes.object, PropTypes.bool]),
  renderModalBody: PropTypes.oneOfType([PropTypes.bool, PropTypes.func]).isRequired,
