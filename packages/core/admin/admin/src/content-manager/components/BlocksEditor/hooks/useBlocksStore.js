    ...
    paragraph: {
      ...
      handleEnterKey(editor) {
        ...
        Transforms.splitNodes(editor, {
          // Always create a new node, even if there's nothing after the cursor in the block
          always: true,
        });

        /**
         * Convert the newly created node to a paragraph. And avoid node pollution
         * (e.g. keeping the level attribute when converting a heading to a paragraph).
         * To do this, set all the attributes to null, except type and children.
         * Slate will auto-remove the null attributes.
         */
        const [createdNode] = Editor.parent(editor, editor.selection.anchor.path);
        const { type, children, ...extraAttributes } = createdNode;
        const attributesToNullify = {};
        Object.keys(extraAttributes).forEach((key) => {
          attributesToNullify[key] = null;
        });
