...
describe('getRelationLink', () => {
  test('returns an URL containing the targetModel and id', () => {
    expect(getRelationLink('model', 2)).toBe('/content-manager/collection-types/model/2');
  });

  test('returns an URL containing the targetModel', () => {
    expect(getRelationLink('model', undefined)).toBe('/content-manager/collection-types/model/');
  });
});
