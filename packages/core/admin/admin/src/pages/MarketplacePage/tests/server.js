...
import { responses } from './mocks/plugins';
...
const handlers = [
  rest.get('https://market-api.strapi.io/plugins', (req, res, ctx) => {
    ...
    if (category && collection) {
      responseData = {
        data: [...responses[collection].data, ...responses[category].data],
        meta: responses.plugins.meta,
      };
    } else if (madeByStrapi && verified) {
      responseData = {
        data: [...responses[madeByStrapi].data, ...responses[verified].data],
        meta: responses.plugins.meta,
      };
    } else if (customFields && monitoring) {
      responseData = {
        data: [...responses[customFields].data, ...responses[monitoring].data],
        meta: responses.plugins.meta,
      };
    } else if (collection) {
      responseData = responses[collection];
    } else if (category) {
      responseData = responses[category];
    } else {
      responseData = responses.plugins;
    }
