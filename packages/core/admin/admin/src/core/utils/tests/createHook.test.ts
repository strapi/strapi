...
describe('ADMIN | core | utils | createHook', () => {
  ...
  it('calls all of the mocks in a waterfall fashion', () => {
    hooksContainer.register((n: number) => n + 1);
    hooksContainer.register((n: number) => n + 2);
    hooksContainer.register((n: number) => n * 3);
  ...
  it('calls all of the mocks in a waterfall fashion when they resolve async code', async () => {
    hooksContainer.register((n: number) => Promise.resolve(n + 1));
    hooksContainer.register((n: number) => n + 2);
    hooksContainer.register((n: number) => Promise.resolve(n * 3));
