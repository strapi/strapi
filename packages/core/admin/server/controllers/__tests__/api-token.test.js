...
describe('API Token Controller', () => {
  ...
  describe('API tokens permissions layout', () => {
    const layout = [
      {
        label: 'address',
    ...
    test('return API tokens layout successfully', async () => {
      const getApiTokenLayout = jest.fn().mockResolvedValue(layout);
      const send = jest.fn();
      const ctx = createContext({}, { send });
      ...
      await apiTokenController.getLayout(ctx);

      expect(getApiTokenLayout).toHaveBeenCalled();
      expect(send).toHaveBeenCalledWith({ data: layout });
    });
  });
});
