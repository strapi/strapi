...
const relativize = (value: string, appRoot?: string): string => {
...
const walk = (value: unknown, appRoot?: string): unknown => {
  if (typeof value === 'string') {
    if (looksSecret(value)) {
      return REDACTED;
    }
    return relativize(value, appRoot);
  }
  if (Array.isArray(value)) {
    return value.map((item) => walk(item, appRoot));
  }
  if (isPlainObject(value)) {
    ...
    for (const [key, val] of Object.entries(value)) {
      ...
      out[key] = SECRET_KEY_PATTERN.test(key) ? REDACTED : walk(val, appRoot);
    }
    return out;
  }
...
export const scrub = (value: unknown, options: ScrubOptions = {}): unknown => {
  const cloned = walk(value, options.appRoot);
