...
const watch = async (ctx: BuildContext) => {
  ...
  ctx.logger.debug('Final rspack config:', os.EOL, finalConfig);

  return new Promise((res) => {
    const compiler = rspack(finalConfig) as Compiler;

    // @ts-expect-error – this is the RSPack compiler, not the webpack compiler
