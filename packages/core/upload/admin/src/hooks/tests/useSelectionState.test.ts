...
describe('useSelectionState', () => {
  ...
  test('selectOne', async () => {
    ...
    await waitFor(() => expect(result.current[0]).toStrictEqual([FIXTURE[0]]));

    act(() => {
      result.current[1].selectOne(FIXTURE[1]);
    });

    await waitFor(() => expect(result.current[0]).toStrictEqual(FIXTURE));
  });
  ...
  test('selectOne deselect', async () => {
    ...
    await waitFor(() => expect(result.current[0]).toStrictEqual([FIXTURE[1]]));

    act(() => {
      result.current[1].selectOne(FIXTURE[1]);
    });

    await waitFor(() => expect(result.current[0]).toStrictEqual([]));
  });
  ...
  test('selectMultiple - already selected item + multiple keys', async () => {
    ...
    await waitFor(() => expect(result.current[0]).toStrictEqual([alreadySelectedItem, ...FIXTURE]));
  });
