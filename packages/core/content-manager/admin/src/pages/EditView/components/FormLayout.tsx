...
import { InputRenderer, InputRendererTest } from './InputRenderer';
...
const FormLayoutTest = ({ layout, hasBackground = true, model: modelProp }: FormLayoutProps) => {
  const { formatMessage } = useIntl();
  const { model: modelUseDoc } = useDoc();
  const model = modelProp || modelUseDoc;
...
export { FormLayout, FormLayoutTest, FormLayoutProps };
