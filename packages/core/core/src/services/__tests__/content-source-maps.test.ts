...
const mockEncodeField = (
  ...
  if ((metadata as any).model) strapiSource.set('model', (metadata as any).model);
  if ((metadata as any).kind) strapiSource.set('kind', (metadata as any).kind);
  if ((metadata as any).locale) strapiSource.set('locale', (metadata as any).locale);
...
describe('encodeBlocks (pure encoder)', () => {
  ...
  test('encodes headings at every level using the first text leaf', () => {
    ...
    encoded.forEach((block, index) => {
      expect((block as any).children[0].text).toContain(`heading ${index + 1}`);
    });
  });
  ...
  test('encodes the first text leaf of each list item in flat lists', () => {
    ...
    expect((encoded[0].children[0] as any).children[0].text).toContain('First item');
    expect((encoded[0].children[1] as any).children[0].children[0].text).toContain('Second item');
    expect((encoded[1].children[0] as any).children[0].text).toContain('Alpha');
    expect((encoded[1].children[1] as any).children[0].text).toContain('Beta');
  });
  ...
  test('encodes nested list items including nested lists', () => {
    ...
    expect((encoded[0].children[0] as any).children[0].text).toContain('Outer item');
  ...
  test('returns empty arrays unchanged', () => {
    expect(encodeBlocks([], baseMetadata, mockEncodeField as any)).toEqual([]);
  });

  test('does not encode when the blocks value is not an array', () => {
    expect(encodeBlocks(null, baseMetadata, mockEncodeField as any)).toBeNull();
  });
  ...
  test('encodes deeply nested inline content using the first text leaf only', () => {
    ...
    expect((encoded[0].children[0] as any).children[0].text).toContain('Read more');
