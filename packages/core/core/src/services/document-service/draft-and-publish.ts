...
/**
 * Parses and sanitizes the deprecated `hasPublishedVersion` parameter.
 * Replaced by `publicationFilter` for REST and document service queries.
 *
 * @deprecated Use `parsePublicationFilter` / `publicationFilter` instead.
 */
...
/**
 * Synchronous helper for the deprecated `hasPublishedVersion` boolean (documentId subquery).
 * Replaced by `getPublicationFilterCondition` / `publicationFilter` cohort modes.
 *
 ...
 *
 * @deprecated Prefer `getPublicationFilterCondition` with `publicationFilter`.
 */
