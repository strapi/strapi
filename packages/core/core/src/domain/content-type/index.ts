...
import { yup, contentTypes as contentTypesUtils, structuredCloneSafe } from '@strapi/utils';
...
const createContentType = (uid: string, definition: ContentTypeDefinition) => {
  ...
  const { schema, actions, lifecycles } = structuredCloneSafe(definition);
...
const pickSchema = (model: Schema.ContentType) => {
  const schema = structuredCloneSafe(
    _.pick(model, [
      'connection',
      'collectionName',
      'info',
