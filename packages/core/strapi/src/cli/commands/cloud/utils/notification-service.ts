...
const LOGGABLE_EVENTS = ['deploymentFinished', 'deploymentFailed'];
const CONN_TIMEOUT = 5 * 60 * 1000; // 5 minutes

const notificationServiceFactory = ({ logger }: CLIContext) => {
  return (url: string, token: string) => {
    return new Promise((resolve, reject) => {
      ...
      es.onmessage = (event: Event) => {
        ...
        if (LOGGABLE_EVENTS.includes(data.event) && data.message) {
