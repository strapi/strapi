...
import { omitExcludedOptimizeDepsPresentInInclude } from '../core/admin-vite-optimize-deps';
...
const resolveBaseConfig = async (ctx: BuildContext): Promise<InlineConfig> => {
  ...
  // Never put the same package in both include and exclude — esbuild throws (#27202).
  const include = [
    ...
    // Omit when linked so local changes are picked up (see exclude below)
     ...
     */
    ...(isMonorepoExampleApp ? [...ADMIN_VITE_MONOREPO_EXAMPLE_OPTIMIZE_INCLUDE] : []),
  ];

  const exclude = omitExcludedOptimizeDepsPresentInInclude(include, [
    ...(designSystemLinked ? ['@strapi/design-system'] : []),
