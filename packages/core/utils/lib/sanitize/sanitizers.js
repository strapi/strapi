'use strict';

const { curry } = require('lodash/fp');

const traverseEntity = require('../traverse-entity');
const { removePassword, removePrivate, composeVisitors } = require('./visitors');

const sanitizePasswords = curry((schema, entity) => {
  return traverseEntity(removePassword, { schema }, entity);
});

const sanitizePrivates = curry((schema, entity) => {
  return traverseEntity(removePrivate, { schema }, entity);
});

const defaultSanitizeOutput = curry((schema, entity) => {
  const visitors = composeVisitors([removePassword, removePrivate]);
  return traverseEntity(visitors, { schema }, entity);
});

module.exports = {
  sanitizePasswords,
  sanitizePrivates,
  defaultSanitizeOutput,
};
