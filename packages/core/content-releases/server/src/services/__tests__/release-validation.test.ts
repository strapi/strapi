...
describe('Release Validation service', () => {
  describe('validateEntryData', () => {
    ...
    it('throws an error if the content type does not have draftAndPublish enabled', () => {
      ...
      expect(() =>
        releaseValidationService.validateEntryData(
    ...
    it('throws an error if the entryType is collection-types and the entryDocumentId is missing', () => {
      const strapiMock = {
        ...baseStrapiMock,
        contentType: jest.fn().mockReturnValue({
          options: {
