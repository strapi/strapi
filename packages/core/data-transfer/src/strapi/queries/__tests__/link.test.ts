...
describe('createLinkQuery', () => {
  const createQueryBuilderMock = ({
    ...
    filterExists?: (row: Record<string, unknown>) => boolean;
    annotateExists?: (row: Record<string, unknown>) => Record<string, unknown>;
  }) => {
    ...
    const qb: Record<string, unknown> = {
        ...
        if (
          flat.some(
            (arg) =>
              arg &&
              typeof arg === 'object' &&
              ((arg as { isRaw?: boolean }).isRaw ||
                '__target_exists' in (arg as Record<string, unknown>))
          )
        ) {
          selectExists = true;
        }
      ...
      whereExists: jest.fn(function whereExistsFn() {
        whereExists = true;
        return this;
      }),
      ...
      then(resolve: (value: unknown) => void) {
        if (whereExists && filterExists) {
          resolve(rows.filter(filterExists));
          return;
        }

        if (selectExists && annotateExists) {
          resolve(rows.map(annotateExists));
          return;
        }
  ...
  test('does not export joinColumn links when the relation target is missing', async () => {
    const connection = {
      client: { connectionSettings: {} },
      raw: jest.fn((sql: string, bindings?: unknown[]) => ({ sql, bindings, isRaw: true })),
      queryBuilder: () =>
        createQueryBuilderMock({
    ...
    const connection = {
      ...
      queryBuilder: () =>
        createQueryBuilderMock({
          ...
          filterExists: (row) => row.document_id === 'doc-1',
        }),
    };
  ...
  test('does not duplicate joinColumn links when referencedColumn matches multiple rows', async () => {
    ...
    const connection = {
      ...
      queryBuilder: () =>
        createQueryBuilderMock({
          ...
          // A JOIN on document_id against two matching target locales would yield 4 rows.
          // EXISTS must preserve owner cardinality (exactly one result per owner row).
          annotateExists: (row) => ({
            ...row,
            __target_exists: row.document_id === 'doc-1',
          }),
        }),
    };
  ...
  test('treats morph targets with unknown UIDs as orphaned', async () => {
    const onOrphanedLink = jest.fn();
    ...
    const morphRows = [
      {
        entity_id: 1,
        related_id: 10,
        related_type: 'component::removed.block',
        field: 'blocks',
        order: 1,
      },
      {
    ...
    const knownUids = new Set(['plugin::upload.file', 'api::article.article']);

    const strapi = {
      db: {
        connection,
        metadata: {
          has: jest.fn((uid: string) => knownUids.has(uid)),
          get: jest.fn((uid: string) => {
            ...
            if (uid === 'api::article.article') {
              return { tableName: 'articles', attributes: {} };
            }
    ...
    expect(links).toEqual([
      expect.objectContaining({
        right: expect.objectContaining({ type: 'api::article.article', ref: 11 }),
      }),
    ]);
    expect(onOrphanedLink).toHaveBeenCalledTimes(1);
    expect(onOrphanedLink).toHaveBeenCalledWith(
      expect.objectContaining({
        right: expect.objectContaining({ type: 'component::removed.block', ref: 10 }),
      })
    );
    expect(findMany).toHaveBeenCalledTimes(1);
    expect(strapi.db.query).not.toHaveBeenCalledWith('component::removed.block');
  });
});
