...
export default (db: Database) => {
  ...
  const diffForeignKeys = (oldForeignKey: ForeignKey, foreignKey: ForeignKey): ForeignKeyDiff => {
    ...
    if (oldForeignKey.onDelete == null || oldForeignKey.onDelete.toUpperCase() === 'NO ACTION') {
      if (
    ...
    } else if (oldForeignKey.onDelete.toUpperCase() !== (foreignKey.onDelete ?? '').toUpperCase()) {
      changes.push('onDelete');
    }

    if (oldForeignKey.onUpdate == null || oldForeignKey.onUpdate.toUpperCase() === 'NO ACTION') {
      if (
    ...
    } else if (oldForeignKey.onUpdate.toUpperCase() !== (foreignKey.onUpdate ?? '').toUpperCase()) {
      changes.push('onUpdate');
    }
  ...
  const diffDefault = (oldColumn: Column, column: Column) => {
    ...
    if (oldDefaultTo === null || _.toLower(oldDefaultTo) === 'null') {
      return defaultTo == null || _.toLower(defaultTo) === 'null';
    }
    ...
    for (const databaseTable of databaseSchema.tables) {
      ...
      if (!isInUserSchema && wasTracked && !isReserved) {
        const dependencies = persistedTables
          ...
          .filter((table: PersistedTable) => table != null);
