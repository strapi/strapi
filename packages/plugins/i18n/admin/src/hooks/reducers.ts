  ...
  switch (action.type) {
    ...
    case ADD_LOCALE: {
      if (action.newLocale.isDefault) {
        draftState.locales.forEach((locale: any) => {
    ...
    case DELETE_LOCALE: {
      const locales = draftState.locales.filter((locale: any) => locale.id !== action.id);
    ...
    case UPDATE_LOCALE: {
      ...
      const indexToEdit = draftState.locales.findIndex(
        (locale: any) => locale.id === action.editedLocale.id
      );
