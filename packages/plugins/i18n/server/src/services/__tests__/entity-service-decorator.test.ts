...
const { decorator } = entityServiceDecoratorFactory();
const localizations = localizationsServiceFactory();
...
describe('Entity service decorator', () => {
  beforeAll(() => {
    global.strapi = {
      ...
      getModel(uid: keyof typeof models) {
  ...
  describe('findMany', () => {
    ...
    test('calls db.findMany for localized collection type', async () => {
      ...
      global.strapi = {
        ...global.strapi,
        getModel: jest.fn((uid: keyof typeof models) => models[uid || 'test-model']),
        db,
      } as any;
