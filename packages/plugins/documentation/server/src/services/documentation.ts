...
import { mergeInfoConfig } from './helpers/utils/merge-info-config';
...
const DEFAULT_DOCUMENTATION_VERSION = '1.0.0';
...
const createService = ({ strapi }: { strapi: Core.Strapi }) => {
  ...
  return {
    getDocumentationVersion() {
      return config.info?.version ?? DEFAULT_DOCUMENTATION_VERSION;
    },
    ...
    async generateFullDoc(versionOpt?: string) {
      ...
      const generatedDocumentation = await produce(config, async (draft) => {
        ...
        draft.info = mergeInfoConfig(config.info);
