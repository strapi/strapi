...
const { buildContentTypeEnumsMap } = require('./enum');

const buildContentTypes = contentTypes => {
  return contentTypes.map(buildContentType).reduce(merge, {});
};

const buildContentType = contentType => {
  const builders = [buildContentTypeEnumsMap, buildEntityAttributes];
  const typesCollections = builders.map(builder => builder(contentType));

  return typesCollections.reduce(merge, {});
};

const buildEntityAttributes = contentType => {
...
module.exports = { buildContentTypes };
