...
export const wrapControllerForVisibility = (
  strapi: Core.Strapi,
  { contentTypeUid, routes, validateRemoval, extraBodyFields = [] }: ControllerWrapOptions
): void => {
  const middleware = async (ctx: any, next: () => Promise<any>) => {
    ...
    if (matched.isUpdate && spacesSlug !== undefined && validateRemoval) {
      ...
      if (resourceId !== null) {
        ...
        if (currentResource) {
          const currentSlugs: string[] = (currentResource.spaces ?? []).map((s: any) => s.slug);
          ...
          if (currentSlugs.length === 0) {
            if (spacesSlug.length === 0) {
              ...
              const allSpaces = await strapi.db.query(SPACES_CT_UID).findMany({ select: ['slug'] });
    ...
    if (spacesSlug !== undefined) {
      const allSpaces = await strapi.db.query(SPACES_CT_UID).findMany({ select: ['id', 'slug'] });
  ...
  // TODO(spaces): each wrapped resource currently installs its own always-on
  // Koa middleware. One (locales) is fine; before a second resource adopts the
  // pattern (tokens, webhooks…), fold these into a single dispatcher
  // middleware over a route registry so the per-request cost stays constant.
...
export const scopeReadByCurrentSpace = (
  ...
  for (const methodName of methods) {
    ...
    service[methodName] = async (params: any = {}, ...rest: any[]) => {
      ...
      const spacesById = new Map(withSpaces.map((row: any) => [row.id, row.spaces ?? []]));
