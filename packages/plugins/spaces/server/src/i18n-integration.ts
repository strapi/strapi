...
export const patchI18nForSpaces = async (strapi: Core.Strapi) => {
    ...
    extraBodyFields: [
      {
        ...
        onAfterWrite: async (strapi, { resourceId, finalSlugs, requestedFor, resource }) => {
          ...
          const isBound = (slug: string) => finalSlugs.length === 0 || finalSlugs.includes(slug);
          ...
          const targetSlugs = finalSlugs.length === 0 ? allSpaces.map((s) => s.slug) : finalSlugs;
  ...
  if (typeof localesService.setDefaultLocaleStrategy === 'function') {
    ...
    localesService.setDefaultLocaleStrategy({
      async get() {
        ...
        const platform = (await i18nStoreClosure.get({ key: 'default_locale' })) as string | null;
        const candidate = perSpace ?? platform ?? null;

        const visible = (await localesServiceClosure.find()) as Array<{ code: string }> | undefined;
      ...
      // NOTE: no per-code `isDefault` — i18n's `setIsDefault` derives the flag by
      // comparing each row against ONE `get()` call (identical resolution), which
      // keeps GET /i18n/locales at a constant number of queries instead of
      // O(rows × store reads).
      ...
      async listDefaults() {
        const allSpaces = (await runUnscoped(() =>
          strapi.db.query('plugin::spaces.space').findMany({ select: ['slug'] })
  ...
  const i18nPermissionsActions = (strapi.plugin('i18n').service('permissions') as any)?.actions;
  if (
    i18nPermissionsActions &&
    typeof i18nPermissionsActions.syncSuperAdminPermissionsWithLocales === 'function'
  ) {
    ...
    // NOTE: do NOT call the sync eagerly here. Plugin bootstrap runs before the
    // admin module registers its actions and resets super-admin permissions
    // (admin/server/src/bootstrap.ts → resetSuperAdminPermissions), so an eager
    // run sees an almost-empty action provider: every permission row errors in
    // `appliesToProperty`, gets dropped from the re-assert list, and
    // `assignPermissions` *deletes* the dropped rows (admin's later reset then
    // recreates them — net effect: hundreds of skip logs and a pointless
    // delete/recreate churn on every boot). The wrapped sync only needs to run
    // on its natural triggers (locale afterCreate/afterDelete), when the
    // provider is fully populated; historic space-narrowed rows are repaired by
    // admin's own reset on every boot, with all locales attached via i18n's
    // `willResetSuperAdminPermissions` hook running unscoped.
  }
};
