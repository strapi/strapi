...
/** Whether the ACTIVE workspace allows moving entries (capability). */
const useCanMoveFromCurrentWorkspace = (): boolean => {
  const { data: spaces } = useGetMineSpacesQuery();
  const current = spaces?.find((s) => s.slug === getCurrentSpaceSlug());
  return { ...DEFAULT_CAPABILITIES, ...(current?.capabilities ?? {}) }.moveEntries;
};
...
export const MoveToSpaceHeaderAction: DocumentActionComponent = ({
  ...
  const canMoveHere = useCanMoveFromCurrentWorkspace();

  if (!documentId || !document) return null;
  if (!allowedActions.canMoveEntry || !canMoveHere) return null;
