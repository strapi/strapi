...
import oauth1 from '../oauth-connect/oauth1';
import oauth2 from '../oauth-connect/oauth2';
import {
  redirectWithPayload,
  buildProviderConfig,
  createOAuthConnectMiddleware,
} from '../oauth-connect';
import * as upUtils from '..';

jest.mock('..', () => {
  const actual = jest.requireActual('..');

  return { ...actual, getService: jest.fn() };
});

const mockProvidersService = () => {
  upUtils.getService.mockImplementation((name) => {
    if (name === 'providers') {
      return {
