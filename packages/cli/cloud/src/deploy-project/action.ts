...
import type { CLIContext, CloudApiService, CloudCliConfig, EnvironmentDetails, ProjectInfo } from '../types';
...
async function hasPendingOrLiveDeployment(environments: EnvironmentDetails[], targetEnvironment: string): Promise<boolean> {
...
export default async (ctx: CLIContext, opts: CmdOptions) => {
  ...
  if (!opts.force) {
    const shouldDisplayWarning = await hasPendingOrLiveDeployment(environmentsDetails, project.targetEnvironment);
