...
| `admin.public` | Add public url for WDS. Only used along with `strapi develop --watch-admin`. See more [here](https://webpack.js.org/configuration/dev-server/#devserverpublic). | string | `localhost` |
| `admin.allowedHosts` | Add allowed hosts for WDS. Only used along with `strapi develop --watch-admin`. See more [here](https://webpack.js.org/configuration/dev-server/#devserverallowedhosts). | Array(string) | `[]` |
