...
- `populateCreatorFields`: Configure whether or not the API response should include `created_by` and `updated_by` fields. Accepts a `boolean`. The default value is `true`.
