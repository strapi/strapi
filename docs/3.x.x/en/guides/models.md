...
> Note: If you are just starting out it is very convinient to generate the some models with the Content Type Builder in the admin interface. You can review the generated model mappings on the code level. The UI takes over a lot of validation tasks and gives you a fast feeling for available features. 
