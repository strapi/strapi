...
Because every registry can be augmented (publicly exported), this means that developers have the freedom to inject their own definitions.
...
To define `UID.ContentType`, we extract every key (`Internal.Registry.Keys`) from the public content-type registry (`Public.ContentTypeSchemas`) that match with the base definition of a content-type UID (`Internal.UID.ContentType`).
...
:::note
Only picking-up keys that extend `Internal.UID.ContentType` ensure we don't end up with manually added malformed keys, and tells the type-checker we're confident about what's in our union type.
:::
...
To create types for a dynamic API (_aka one that react to its context_), simply use dynamic type definitions such as `UID`, `Data` or `Schema`.
...
:::info[Reminder]
Registries are **indexed**, it means that:
...
This will force every type that depend on the `Public.ContentTypeSchemas` registry to recognize `'api::article.article'` as the only valid UID and `ApiArticleArticle` the only valid schema.

:::note
In the context of a Strapi application, developers are encouraged to use types exported by `@strapi/strapi` and not `@strapi/types`.
