...
In the Type System, UIDs play a crucial role in referencing various resource (among which schema and entities) by attaching them a unique identifier.
...
There are two main family of namespaces:
...
Scoped namespace are defined by a base name, followed by a separator (`::`) and any string.
...
These namespaces are simple prefix and define the origin of a resource.
...
A common usage is to declare a function that takes a UID as parameter.
