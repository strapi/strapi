...
Schema types represents **loaded** schemas in the context of a Strapi server application and should be used accordingly.

:::caution
Database models and schema definitions (before being loaded by Strapi) are not the same types and can't be used interchangeably.
:::
...
In between each box is a text that represents the discriminant used to differentiate the subtype from others.
...
Schema types contains useful information that helps other types know how to interact with the Strapi content.
...
Strongly typed schema attribute allows the Type System to infer actual entities types based on their properties.
...
When designing public APIs (and in most other scenario), it's advised to use the high-level schema types found in the `Schema` namespace.
