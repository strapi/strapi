...
Whilst we are actively developing V4 of the CMS we should _always_ be using TanStack Query's hooks to
manage the fetching and mutation of data in conjunction with `useFetchClient`. This is because
TanStack Query caches our data-fetching meaning multiple calls are not necessarily made unless a
mutation occurs as well as handling various intermittent states and events during the fetching
lifecycle.

:::note
We're using `@tanstack/react-query` version 4. See the [v4 documentation](https://tanstack.com/query/v4/docs/framework/react/overview).
:::
...
You can read more about [TanStack Query v4 query keys](https://tanstack.com/query/v4/docs/framework/react/guides/query-keys).
