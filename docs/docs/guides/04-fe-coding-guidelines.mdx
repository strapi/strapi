...
Use the data-fetching layer already established by the area you are changing:

- Admin core uses [RTK Query](https://redux-toolkit.js.org/rtk-query/overview). Define endpoints by
  enhancing and injecting them into `adminApi`, then consume the generated hooks.
- Some plugins and legacy admin surfaces still use
  [TanStack Query v4](https://tanstack.com/query/v4/docs/framework/react/overview) with
  `useFetchClient`.

Do not create a TanStack query for an endpoint already managed by `adminApi`; that creates a second,
independent cache. When working in a TanStack surface, keep these query-key guidelines:

- Use URL fragments from the called URL, for example
  `/admin/content-manager/:id/stages` → `['content-manager', id, 'stages']`, except for `admin`.
- Prefix Enterprise Edition calls with `ee` so they can be invalidated together.
- Include every dynamic variable in the query key.

Read more about [TanStack Query v4 query keys](https://tanstack.com/query/v4/docs/framework/react/guides/query-keys).
...
const useLicenseLimits = () => {
  const { data, isError, isLoading } = useGetLicenseLimitsQuery();

  return { license: data?.data, isError, isLoading };
};
```
...
const MyPage = () => {
	...
	const { isLoading, data } = useQuery(['my-data'], async () => {
