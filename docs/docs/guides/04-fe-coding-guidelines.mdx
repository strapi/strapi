...
The contributor docs are not “user” docs, they're related directly to the codebase for both internal
and external engineers written with that in mind they explain technical concepts but also hold
documentation for hooks, utils etc.

#### Structure

Currently the documentation is split into four sections:
...
The `Docs` section is organised to model the monorepo only to the package level e.g. `core > admin`
past that point, they're organic and re-organise as we continue to grow the documentation. The `API`
section at time of writing is light and only discusses the `Strapi` class. Moving forward, we'd like
to include all the `admin` API endpoints and their return values.
