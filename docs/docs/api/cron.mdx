...
- If the value is an object, its `task` property is used as the task function, and its `options` property is used as the schedule: a cron `string`, a one-shot `Date` (or timestamp `number`), or `{ rule, tz?, start?, end? }` (the documented timezone form).
...
- <Type>Array</Type>
  ...
  - <Type>Object</Type>
    ...
    - `options`: <Type>String</Type> | <Type>Number</Type> | <Type>Date</Type> | <Type>Object</Type> - Cron string, timestamp, one-shot `Date`, or `{ rule, tz?, start?, end? }`
