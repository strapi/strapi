...
- If the value is an object, its `task` property is used as the task function, and its `options` property is used as the schedule: a cron `string`, a one-shot `Date` (or timestamp `number`), `{ rule, tz?, start?, end? }`, or a node-schedule-style recurrence object (`{ dayOfWeek, hour, minute, … }`). `start`/`end` may be `Date`, timestamp, or ISO string. The task is called as `task({ strapi }, fireDate)`.
...
- <Type>Array</Type>
  ...
  - <Type>Object</Type>
    ...
    - `job`: [<Type>Cron</Type>](https://github.com/Hexagon/croner) - Croner job, with node-schedule aliases `invoke`, `cancel`, `nextInvocation`, and `reschedule`
    - `options`: <Type>String</Type> | <Type>Number</Type> | <Type>Date</Type> | <Type>Object</Type> - Cron string, timestamp, one-shot `Date`, `{ rule, tz?, start?, end? }`, or a recurrence object
