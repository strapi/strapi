...
This hook also wraps an internal hook `useKeyboardDragAndDrop` which implements keyboard accessible drag and drop by
...
adds an accessible label from the `aria-label` prop. So we don't have to concern ourselves with that.
