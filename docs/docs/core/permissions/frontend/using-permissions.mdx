...
If the hook is unmounted before the fetch request completes, the request will be cancelled via an [`AbortController`](https://developer.mozilla.org/en-US/docs/Web/API/AbortController).
...
import { useSyncRBAC } from 'path/to/content-manager/hooks';
