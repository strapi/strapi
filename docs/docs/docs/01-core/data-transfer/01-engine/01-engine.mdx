...
To skip upload binaries as well, combine with `--exclude files`. Programmatic transfers can use the same rules via entity and link transform filters.

#### Excluding the media library (issue #25008)

Users who want export/import to omit **all** media — both upload binaries and media library database records — should combine both flags. `--exclude files` alone only skips the **assets** stage; media library records still transfer unless you also exclude their content types.
...
yarn strapi import -f export_YYYYMMDDHHMMSS.tar --force --exclude files \
  --exclude-content-types plugin::upload.file,plugin::upload.folder
```

npm users: `npm run strapi export -- --no-encrypt --exclude files --exclude-content-types plugin::upload.file,plugin::upload.folder`
