...
:::caution **Destructive Operation**: Currently, the transfer engine is designed as a synchronization tool. When using the default `local-destination` provider, the engine uses a **restore strategy** by default. This will **delete all existing data** (entities, links, assets, and configuration) in the destination database during the bootstrap phase before any new data is written. :::
...
sequenceDiagram
    ...
    rect rgb(230, 245, 230)
        Note over Client,DP: 1️⃣ Bootstrap & Initialize Phase (Destructive)
        Client->>+TE: Start Transfer
        TE->>SP: Bootstrap
        Note right of SP: Connect to files,<br/>databases, etc.
        TE->>DP: Bootstrap
        Note right of DP: Setup connections &<br/>WIPE DESTINATION DATA (Restore)
        TE->>SP: Get Metadata
...
### The Bootstrap Phase and Data Loss

A transfer starts by bootstrapping and initializing itself and the providers. In this stage, providers establish connections to the database and file systems.

**Important:** For the `local-destination` provider, the `bootstrap` method checks the transfer strategy. Currently, `restore` is the only supported strategy for full transfers. When this is active, the provider executes a internal `#deleteAll()` method which truncates all tables and removes all assets in the destination before the transfer begins. This ensures no ID conflicts occur during the `entities` and `links` stages, but it means the operation is **non-reversible** and will result in the loss of all pre-existing data on the destination instance.
