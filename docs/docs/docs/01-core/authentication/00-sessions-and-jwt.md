...
- Cookie options:
  ...
  - `admin.auth.cookie.path` (default `/admin`) — applied to the non-httpOnly access-token cookie written by the admin client (and EE SSO) only. Requires an admin rebuild after change (value is inlined into the admin bundle at build time). Use a distinct path per instance when hosting multiple Strapis under the same parent domain. Note that existing `cookie.path` customizations no longer move the refresh cookie.
  - `admin.auth.cookie.refreshPath` (default `/admin`) — path scope applied to `strapi_admin_refresh`. Independent of `path`, resolved at runtime, and not inlined into the admin bundle. Existing `cookie.path` customizations no longer move the refresh cookie; configure this setting if running behind reverse-proxy prefixes (such as `/strapi-de/admin`) where the browser path to `/admin/access-token` differs from `/admin`.
