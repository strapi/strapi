...
1. `entry` is a fully populated entry. If you are validating a Content Type with required fields that are relations/components/dynamic zones, you need to send these fields populated in the object. Otherwise, the validation will fail.
