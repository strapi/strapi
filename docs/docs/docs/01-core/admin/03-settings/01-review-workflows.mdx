...
from a list of all content-types (collections as well as single-types) for each workflow. In case a content-type is already assigned to another content-type, the content-type
...
The form wrapping both views submits all stages at once, because we expect the number of stages per workflow to be
