...
module.exports = function isLocalStrapiValid(strapiPath, appPath) {
  ...
  // Current Node.js version is greater than 5.12.0.
  if (semver.gt(process.version, '4.8.0')) {
    self.log.warn('Oups..., we cannot start the server.');
    self.log.warn('Your Node.js version is greater than 4.8.0.');
    self.log.warn('Please use downgrade your Node.js version.');
    // Force exit.
    process.exit(0);
  }
