...
module.exports = function (strapi) {
  ...
  return function initializeHooks(hooks, cb) {
    ...
    async.series({
      ...
      // Load the user modules dictionary.
      _modules: function loadModulesHook(cb) {
        if (!hooks._modules) {
          return cb();
        }
        prepareHook('_modules');
        applyDefaults(hooks._modules);
        loadHook('_modules', cb);
      },
      ...
      prepare: function prepareHooks(cb) {
        async.each(_.without(_.keys(hooks), '_config', '_api', '_modules', '_admin'), function (id, cb) {
          prepareHook(id);
          process.nextTick(cb);
        }, cb);
      },
